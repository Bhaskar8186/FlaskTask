...
@token_required
def protected():
    return jsonify({'message' : 'Hello DB.'})
...
def login():
    ...
    if   auth.password == 'DB@2020':

        token = jwt.encode({'user': auth.username, 'exp': datetime.datetime.utcnow() + datetime.timedelta(seconds=180)}, app.config['SECRET_KEY'])
